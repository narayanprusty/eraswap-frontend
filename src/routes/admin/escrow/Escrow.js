...
class Escrow extends React.Component {
  ...
  render() {
    return (
      ...
      <Select defaultValue="EST" style={{ width: 120 }} onChange={this.handleChange.bind(this)}>
    ...
    <br />
    <br />
        Deposit Address :  <code> {this.state.depositAddress || "Loading..."}</code>
        <br />
        Balance: <code> {this.state.balance || "Loading..."}</code>
        ...
        <Input name="amount"
                  ...
                  />
                   <Input
                    ...
                    style={{ width: '25%', marginRight: '3%' }}
                  />
                  {!this.state.loader && (<Button type='primary' onClick={this.withdrawProcess.bind(this)} disabled={this.state.currency && this.state.address && this.state.amount ? false :true}>
                    Send
                  </Button>)}
